...
topology = [
    ...
    (35, 7), (7, 246), (246, 33), (7, 163), (163, 246), (246, 7), (163, 144),
    (144, 161), (161, 163), (144, 145), (145, 160), (160, 144), (145, 153),
    (153, 159), (159, 145), (153, 154), (154, 158), (158, 153), (154, 155),
    (155, 157), (157, 154), (155, 133), (133, 173), (173, 155), (362, 382),
    (382, 398), (398, 362), (382, 381), (381, 384), (384, 382), (381, 380),
    (380, 385), (385, 381), (380, 374), (374, 386), (386, 380), (374, 373),
    (373, 387), (387, 374), (373, 390), (390, 388), (388, 373), (390, 249),
    (249, 466), (466, 390), (249, 263), (263, 466), (466, 249), (78, 95),
    (95, 191), (191, 78), (95, 88), (88, 80), (80, 95), (88, 178), (178, 81),
    (81, 88), (178, 87), (87, 82), (82, 178), (87, 14), (14, 13), (13, 87),
    (14, 317), (317, 13), (13, 14), (317, 402), (402, 312), (312, 317), (402, 318),
    (318, 311), (311, 402), (318, 324), (324, 310), (310, 318), (324, 308), (308, 415),
    (415, 324), (163, 161), (161, 246), (246, 163), (144, 160), (160, 161), (161, 144),
    (145, 159), (159, 160), (160, 145), (153, 158), (158, 159), (159, 153),
    (154, 157), (157, 158), (158, 154), (155, 173), (173, 157), (157, 155),
    (382, 384), (384, 398), (398, 382), (381, 385), (385, 384), (384, 381),
    (380, 386), (386, 385), (385, 380), (374, 387), (387, 386), (386, 374),
    (373, 388), (388, 387), (387, 373), (390, 466), (466, 388), (388, 390)]

triangles = []
for i in range(0, len(topology), 3):
    (point1,_), (point2,_), (point3,_) = topology[i:i+3]
    triangles.append((point1, point2, point3))

canopnical_points = np.load("facemesh_2d_points.npy")

scaler = MinMaxScaler(feature_range=(0,255))
scaled = scaler.fit_transform(canopnical_points)
scaled = np.asarray(scaled).astype(float)

def toTexture(count, image):
    face_mesh = mp_face_mesh.FaceMesh(refine_landmarks=False)
    texture_img = np.zeros((256, 256, 3), np.uint8)
    ...
    if results.multi_face_landmarks:
        for face in results.multi_face_landmarks:            
            for i, triangle in enumerate(triangles):
                ...
                vertex_idx1, vertex_idx2, vertex_idx3 = triangle
                r_vertex1, r_vertex2, r_vertex3 = face.landmark[vertex_idx1], face.landmark[vertex_idx2], face.landmark[vertex_idx3]
                c_vertex1, c_vertex2, c_vertex3 = scaled[vertex_idx1], scaled[vertex_idx2], scaled[vertex_idx3]
                
                (c_x1, c_y1), (c_x2, c_y2), (c_x3, c_y3) = c_vertex1, c_vertex2, c_vertex3
                
                def getRelativePoint(landmark):
                    relative_x = int(landmark.x * shape[1])
                    relative_y = int(landmark.y * shape[0])
                    return relative_x, relative_y
                
                r_x1, r_y1 = getRelativePoint(r_vertex1)
                r_x2, r_y2 = getRelativePoint(r_vertex2)
                r_x3, r_y3 = getRelativePoint(r_vertex3)
                
                src_tri = np.float32([[[r_x1, r_y1], [r_x2, r_y2], [r_x3, r_y3]]])
                t_tri = np.float32([[c_vertex1, c_vertex2, c_vertex3]])
                
                r1 = cv2.boundingRect(src_tri)
                r2 = cv2.boundingRect(t_tri)
                
                tri1cropped = []
                tri2cropped = []
                
                for i in range(0, 3):
                    tri1cropped.append(((src_tri[0, i, 0] - r1[0]), (src_tri[0, i, 1] - r1[1])))
                    tri2cropped.append(((t_tri[0, i, 0] - r2[0]), (t_tri[0, i, 1] - r2[1])))
                    
                img1Cropped = image[r1[1]:r1[1] + r1[3], r1[0]:r1[0] + r1[2]]
                ...
                for i in range(r2[1], r2[1]+r2[3]):
                    for j in range(r2[0], r2[0]+r2[2]):
                        if texture_img[i, j, 0] == 0 and texture_img[i, j, 1] == 0 and texture_img[i, j, 2] == 0:
                            texture_img[i, j] = img2Cropped[i - r2[1], j - r2[0]]
#                         else:
#                             texture_img[i, j] = (texture_img[i, j] + img2Cropped[i-r2[1], j-r2[0]]) // 2
                            
    cv2.imwrite('texture_images/{}.jpg'.format(count), texture_img)

cap = cv2.VideoCapture('test/3_1.mp4')
...
count = [i for i in range(frames.shape[0])]

pool = multiprocessing.Pool(processes=10)
pool.starmap(toTexture, zip(count, frames))
pool.terminate()
pool.join()